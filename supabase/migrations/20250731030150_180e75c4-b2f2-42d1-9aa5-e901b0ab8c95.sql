-- Primeiro, adicionar os campos de cliente à tabela profiles
...
-- Remover a coluna client_id da tabela pets
ALTER TABLE public.pets DROP COLUMN IF EXISTS client_id;

-- Remover a tabela clients (já que os dados foram migrados para profiles)
DROP TABLE IF EXISTS public.clients;

-- Atualizar as policies RLS da tabela pets para usar user_id diretamente
...
-- Atualizar as policies RLS da tabela appointments para usar user_id diretamente
-- (As policies já estão corretas, mas vamos garantir consistência)

-- Remover a tabela clients das foreign keys se houver
-- (não há foreign keys explícitas, mas é bom garantir)